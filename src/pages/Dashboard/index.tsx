...
const Dashboard = () => {
	...
	const saveName = (newName: any) => {
		if (editingId) {
			...
			const isDuplicate = workflows.some(
				(workflow) => workflow.name === newName.trim() && workflow.id !== editingId,
			);
				...
				<div>
					...
					<ul className="mt-5 flex flex-col gap-4">
						{workflows.map((workflow) => (
							<li
								onClick={editingId === workflow.id ? () => { } : () => handleSelectWorkflow(workflow.id)}
								...
								<div>
									{editingId === workflow.id ? (
										<div className="flex gap-2 hover:text-black">
											<Button onClick={() => saveName(newName)} className="px-4 py-2 text-white rounded-md bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-700">
												Save
											</Button>
											<Button onClick={() => setEditingId(null)} className="px-4 py-2 text-white rounded-md bg-red-500 hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-700">
												Cancel
											</Button>
										</div>
